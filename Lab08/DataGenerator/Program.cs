...
namespace DataGenerator
{
    class Program
    {
        ...
        private static List<CartAction> GenerateActions()
        {
            ...
            int itemIndex = Program._random.Next(0, items.Length - 1);
            int stateIndex = Program._random.Next(0, states.Length - 1);

            Array values = Enum.GetValues(typeof(ActionType));
            ActionType randomAction = (ActionType)values.GetValue(Program._random.Next(values.Length));

            var action = new CartAction
            {
                CartId = Program._random.Next(1000, 99999),
                Action = randomAction,
